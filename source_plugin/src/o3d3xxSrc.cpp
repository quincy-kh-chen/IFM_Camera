...
extern "C" {
    ...
    */
    ...
    @param[in] param parameters of plugin instance. Parameters should be 
               "CAMERA_IP_1:PCIC_PORT_NUMBER:XMLRPC_PORT_NUMBER:APPLICATION_ID"
    ...
    */
    pmdDllExport int pmdpUpdate (unsigned hnd)
    {
        ...
        dat->o3d3xxCamera->readResolution (& (dat->frameWidth), & (dat->frameHeight));
        dat->currentDD.img.numColumns = dat->frameWidth;
        dat->currentDD.img.numRows = dat->frameHeight;
    ...
    */
    ...
    @return PMD_OK, if no error occurred, else relevant error code
    */
    pmdDllExport int pmdpGetSourceDataSize (unsigned hnd, uint32_t *result)
    {
        if (!idOk (hnd))
        {
            globalErrorMessage = "unknown handle";
    ...
    */
    pmdDllExport int pmdpSetIntegrationTime (unsigned hnd, unsigned idx, unsigned val)
    {
        ...
        if (idx > 0)
        {
            dat->errorMessage = "Index value cannot be greater than 0" ;
            return PMD_NOT_IMPLEMENTED;
        }

        if ( (status = dat->o3d3xxCamera->setIntegrationTime (val, idx)) != 0)
        {
    ...
    */
    pmdDllExport int pmdpGetValidIntegrationTime (unsigned hnd, unsigned *result, unsigned index, Proximity proximity, unsigned us)
    {
        ...
        {
            *result = 0;
            dat->errorMessage = "pmdpGetValidIntegrationTime: Index cannot be greater than 2" ;
            return PMD_INVALID_VALUE;
        }
        if (us < 0)
        {
    ...
    */
    pmdDllExport int pmdpSetModulationFrequency (unsigned hnd, unsigned idx, unsigned val)
    {
		...
		if (idx != 0)
        {
            dat->errorMessage = "Only Index 0 is supported" ;
            return PMD_NOT_IMPLEMENTED;
        }

		if ( (status = dat->o3d3xxCamera->setModulationFrequencyChannel (val)) != 0)
        {
            dat->errorMessage = "Could not set the rqeuested integration time";
            return PMD_RUNTIME_ERROR;
        }
		dat->currentDD.img.modulationFrequency[idx] = val;
    ...
    */
    pmdDllExport int pmdpGetModulationFrequency (unsigned hnd, unsigned *result, unsigned idx)
    {
		...
		if(idx > 0)
		{
			g_data[hnd]->errorMessage = "Index O only supported";
			return PMD_NOT_IMPLEMENTED;
		}
		
		if ( (status = dat->o3d3xxCamera->getModulationFrequencyChannel (result)) != 0)
        {
            dat->errorMessage = "Could not get the rqeuested freqency channel";
            return PMD_RUNTIME_ERROR;
        }
		dat->currentDD.img.modulationFrequency[idx] = *result;
		return PMD_OK;
    }
    ...
    */
    pmdDllExport int pmdpSourceCommand (unsigned hnd, char *result, size_t maxLen, const char *cmd)
    {
		...
		else if(command == "SetUnambiguousDistanceRange" || command == "SetExposureMode")
		{
			res = dat->o3d3xxCamera->setImagerType( command, param, response );
            res = (res != ERR_NONE) ? PMD_LOGIC_ERROR : PMD_OK;
		}
		else if(command == "OptimizeExposureTime")
		{
			res = dat->o3d3xxCamera->optimize( response );
            res = (res != ERR_NONE) ? PMD_LOGIC_ERROR : PMD_OK;
		}
        ...
        else if (command == "SendPCICommands")
        {
            res = dat->o3d3xxCamera->sendPCICCommand (param, response);  
            if(response == "!")
            {
                res = PMD_LOGIC_ERROR;
            }
            ...
            {
                res = PMD_INVALID_VALUE;
            }
        }
        /* command == "GetDistanceImageFilter" || command == "GetAmplitudeImageFilter" \
			|| command == "GetSpatialFilterType" || command == "GetTemporalFilterType" || \
			command == "GetSpatialFilterMaskSize" || command == "GetTempMeanFilterNumberOfImages" ||\
			command == "GetImagerSettingType" || command == "GetTriggerMode" || \
			command == "GetAmplitudeCorrection" || command == "GetRectificationDistanceImage" || \
			command == "GetRectificationAmplitudeImage" || command == "GetExposureTimeList" \
            command == "GetExposureTimeRatio" || command == "GetImageResolution" || \
            command == "GetClippingCuboid" */
		else if (command.find("Get") != std::string::npos)
		{
			res = dat->o3d3xxCamera->getImagerSettings( command, response );
            res = (res != ERR_NONE) ? PMD_NOT_IMPLEMENTED : PMD_OK;
            // For "GetExosureTimeList", the camera can return string with only 1 value or 2 or 3 values.
            // If there are less than three values in the string append -1 to make it 3 values.
            if(command == "GetExposureTimeList")
            {
                int first_pos = response.find(";");
                ...
                {
                    int second_pos = response.find(";", first_pos + 1);
        ...
        /* command == "EnableFilterDistanceImage" || command == "EnableFilterAmplitudeImage" \
			|| command == "SetSpatialFilterType" || command == "SetTemporalFilterType" || \
			command == "SetSpatialFilterMaskSize" || command == "SetTempMeanFilterNumberOfImages" || \
			command == "EnableAmplitudeCorrection" || command == "EnableRectificationDistanceImage" || \
			command == "EnableRectificationAmplitudeImage" || command == "SetTargetFrameRate" || \
			command == "EnableContinuousAutoExposure" || command == "SetExposureTimeRatio" || \
            command == "SetSourceDataBinSelect"  || command == "SetSourceDataJson" || \
            command == "SetImageResolution" || command == "SetClippingCuboid" */
        else if (command.find("Enable") != std::string::npos || command.find("Set") != std::string::npos)
		{
			res = dat->o3d3xxCamera->setImagerSettings( command, param, response );
            res = (res != ERR_NONE) ? PMD_LOGIC_ERROR : PMD_OK;
		}
