...
struct structImageData
{
    ...
    void *pDiagnosticData;/**< Pointer to start of diagnostic data in frame buffer */
    ...
    size_t diagnosticDataSize;/**< Size of confidence image in bytes */
};
...
extern "C" {
    ...
    */
    ...
    @param[in] param parameters of plugin instance. Currently, this input is not used. Pass in blank string.
    ...
    */
                                  ...
                                  PMDDataDescription *fmtIn, void **input)
    {
        ...
        if(fmtIn[0].DID != dat->did_calculated)
        {
            /* Initialize data pointers to NULL. If a particular image is not received
               from camera, the data pointer will be NULL, and no data copy will be performed */
            dat->frameDataDescriptor.ImageData.pNormalizedAmplitudeImage = NULL;
            dat->frameDataDescriptor.ImageData.pDistanceImage = NULL;
            dat->frameDataDescriptor.ImageData.pXCoordinateImage = NULL;
            dat->frameDataDescriptor.ImageData.pYCoordinateImage = NULL;
            dat->frameDataDescriptor.ImageData.pZCoordinateImage = NULL;
            dat->frameDataDescriptor.ImageData.pConfidenceImage = NULL;

            int res = parseFrameData (hnd, input[dataIdx], 0);
            if (res != PMD_OK) // TODO calculation of lenggth or remove the parameter if not neccessary
            {
                memset (output[0], 0, fmtOut[0].size);
                return PMD_DATA_NOT_FOUND;
            }
            dat->did_calculated = fmtIn[0].DID;
    ...
    */
    pmdDllExport int pmdpProcessingCommand (unsigned hnd, char *result, size_t maxLen, const char *cmd)
    {
        ...
        else if(command == "GetDiagnosticDataSize")
        {
            std::ostringstream diagnostidDataSize ;
            diagnostidDataSize << dat->frameDataDescriptor.ImageData.diagnosticDataSize;
            memcpy (result , diagnostidDataSize.str().c_str(), diagnostidDataSize.str().length());
            return PMD_OK;
        }
        else if(command == "GetDiagnosticData")
        {
            uint32_t *pDiagnosticDataParsing = NULL;
            std::stringstream diagnosticDataStream;
            int32_t value;
            pDiagnosticDataParsing = (uint32_t *) dat->frameDataDescriptor.ImageData.pDiagnosticData;
            value = (int32_t)(* (pDiagnosticDataParsing++));
            if(value != 32767)
                diagnosticDataStream << value;

            for (unsigned i = 1; i < dat->frameDataDescriptor.ImageData.diagnosticDataSize / sizeof(uint32_t); i++)
            {
                value = (int32_t)(* (pDiagnosticDataParsing++));
                // imx6 temparature and frontend 2 temperatures are not measured. Hence, they are set to 0x7FFF(32767)
                // Do not pass out these values. 
                // On change in implementation in SDK or camera, remember to update manual document accordingly.
                if(value != 32767)
                    diagnosticDataStream << ";" << value;
                
            }
            memcpy(result, diagnosticDataStream.str().c_str(), diagnosticDataStream.str().length());
            return PMD_OK;
        }
...
int32_t parseFrameData (unsigned hnd , void *frameData , int length)
{
    ...
    from camera. One by one parse the images in the frame and mark offset of
    starting of each image in frame. */
    while (1)
    {
        memcpy (& (o3d3xxImageHeader), pFrameData , IMAGE_HEADER_SIZE);
        memcpy(stop_str, pFrameData, 4);// 4 characters for "stop".
        if(strcmp(stop_str, "stop") == 0)
            break;
        ...
        switch (o3d3xxImageHeader.chunkType)
        {
            ...
            /*Image is ALL cartesian image*/
            case 203:
                //dat->frameDataDescriptor.ImageData.pZCoordinateImage = pFrameData + IMAGE_HEADER_SIZE;
                break;

            /*Image is ALL Unit vectors image*/
            case 223:
                //dat->frameDataDescriptor.ImageData.pZCoordinateImage = pFrameData + IMAGE_HEADER_SIZE;
                break;
            ...
            /*Image is Raw data image*/
            case 301:
                break;
            
            /*Image is Diagnostic image*/
            case 302:
                dat->frameDataDescriptor.ImageData.pDiagnosticData = pFrameData + IMAGE_HEADER_SIZE;
                dat->frameDataDescriptor.ImageData.diagnosticDataSize = o3d3xxImageHeader.chunkSize - o3d3xxImageHeader.headerSize;
                break;
            /*Image is RAW data MIRA image*/
            case 303:
                
            /*Image is Rawdata MIRA CSI2 image*/
            case 304:
                
            /*Image is Extrinsic Calibration image*/
            case 400:
                
            /*Image is JSON Model image*/
            case 500:
                //dat->frameDataDescriptor.ImageData.pZCoordinateImage = pFrameData + IMAGE_HEADER_SIZE;
                break;
...
int32_t getXYZCoordinateImages (unsigned hnd, float *data, size_t maxLen)
{
    ...
    bool xyz_not_selected_flag = (dat->frameDataDescriptor.ImageData.pXCoordinateImage == NULL) && \
        (dat->frameDataDescriptor.ImageData.pYCoordinateImage == NULL) && (dat->frameDataDescriptor.ImageData.pZCoordinateImage == NULL);

    if(!xyz_not_selected_flag)
    {
#ifdef DEBUG
